...
using NativeWindow = GLFW.NativeWindow;
...
namespace THBimEngine.Presention
{
    ...
    public class GLControl : Control
    {
        ...
        {
            get => Glfw.GetClientApi(_nativeWindow);
            set
            {
                if (value != API)
                {
        ...
        {
            get => Glfw.GetProfile(_nativeWindow);
            set
            {
                if (value != Profile)
                {
        ...
        {
            get => Glfw.GetContextVersion(_nativeWindow);
            set
            {
                if (value != APIVersion)
                {
       ...
       // private NativeInput? _nativeInput;
		...
		private void _nativeWindow_MouseDown1(MouseButtonEventArgs obj)
		{
		...
		private void _nativeWindow_MouseDown(MouseButtonEventArgs obj)
		{
			//throw new NotImplementedException();
		}
        ...
        private unsafe void NonportableReparent(NativeWindow nativeWindow)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                Win32.SetWindowBackground(hWnd, (uint)Win32.WindowStylesEx.WS_EX_ALPHA);
            }
		...
		private void ResizeNativeWindow()
        {
            ...
            if (_nativeWindow != null)
            {
                if (Width < 10 || Height < 10)
                    return;
                _nativeWindow.Bounds = new System.Drawing.Rectangle(Location, new System.Drawing.Size(Width, Height));
            }
        }
        ...
        private void OnNativeWindowFocused(object sender, EventArgs e)
        {
            if (sender == null)
                return;
            var tempWin = sender as NativeWindow;
            if (tempWin == null)
                return;
			if (tempWin.IsFocused)
			{
				ForceFocusToCorrectWindow();
