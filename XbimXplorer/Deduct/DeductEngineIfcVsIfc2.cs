...
{
    internal class DeductEngineIfcVsIfc2
    {
        ...
        public void DeductEngine()
        {
            ...
            var strucWall = ModelList.Where(x => x.Value.ItemType == DeductType.StructWall);
            var archiWall = ModelList.Where(x => x.Value.ItemType == DeductType.ArchiWall && x.Value.ChildItems.Count() > 0);
            var strucStorey = ModelList.Where(x => x.Value.ItemType == DeductType.StructStorey);
            var archiStorey = ModelList.Where(x => x.Value.ItemType == DeductType.ArchiStorey);

            //var storeyWallDict = GetSpIdxIfc2(storeyStruct, out var wallTupleDict);

            //var deductWall = DeductWall(storeyStruct, storeyWallDict);
        ...
        private void BuildStruct2D()
        {
            ...
            foreach (var ifcStorey in storeyStruct)
            {
                ...
                //dmBuilding.Contains.Add(dmStorey.UID);
