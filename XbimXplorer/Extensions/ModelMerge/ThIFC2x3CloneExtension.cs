...
namespace XbimXplorer.Extensions.ModelMerge
{
    public static class ThIFC2x3CloneExtension
    {
        ...
        private static IfcLocalPlacement ToIfcLocalPlacement(this IfcStore model, IfcObjectPlacement relative_to = null)
        {
            return model.Instances.New<IfcLocalPlacement>(l =>
            {
                l.PlacementRelTo = relative_to;
                l.RelativePlacement = model.ToIfcAxis2Placement3D();
            });
        }

        private static IfcAxis2Placement3D ToIfcAxis2Placement3D(this IfcStore model)
        {
            return model.Instances.New<IfcAxis2Placement3D>(p =>
            {
                p.Axis = model.ToIfcDirection(new XbimVector3D(0,0,1));
                p.RefDirection = model.ToIfcDirection(new XbimVector3D(1, 0, 0));
                p.Location = model.ToIfcCartesianPoint(new XbimPoint3D(0, 0, 0));
            });
        }
